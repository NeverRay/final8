...
func (s ParcelStore) SetAddress(number int, address string) error {
	...
	if n, _ := res.RowsAffected(); n == 0 {
		var exists bool
		err := s.db.QueryRow(`SELECT EXISTS(SELECT 1 FROM parcel WHERE number = ?)`, number).Scan(&exists)
		if err != nil {
			return err
		}
		if !exists {
		...
		return fmt.Errorf("cannot change address: parcel %d is not in 'registered' status", number)
	}
...
func (s ParcelStore) Delete(number int) error {
	...
	if n, _ := res.RowsAffected(); n == 0 {
		...
		return fmt.Errorf("cannot delete parcel %d: not in 'registered' status", number)
	}
